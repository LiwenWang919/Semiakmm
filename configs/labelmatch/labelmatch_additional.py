...
data = dict(
    ...
    train=dict(
        type='SemiDataset',
        ann_file=data_root + f'annotations/instances_train2017.json',
        ann_file_u=data_root + f'annotations/semi_supervised/unlabeled2017.json',
        pipeline=pipeline, pipeline_u_share=pipeline_u_share,
        pipeline_u=pipeline_u, pipeline_u_1=pipeline_u_1,
        img_prefix=data_root + 'train2017/', img_prefix_u=data_root + 'unlabeled2017/',
    ),
    val=dict(
        type=dataset_type,
...
# # -------------------------schedule------------------------------
learning_rate = 0.004 * samples_per_gpu * gpu / 32
...
labelmatch_hook_cfg = dict(
    samples_per_gpu=samples_per_gpu,
    workers_per_gpu=4,
    label_file=data_root + f'annotations/instances_train2017.json',
    evaluation=dict(interval=update_interval, metric='bbox', by_epoch=False),
    data=dict(
        type='TXTDataset', img_prefix=data_root + 'unlabeled2017/',
        ann_file=data_root + f'annotations/semi_supervised_txt/unlabeled2017.txt',
...
log_level = 'INFO'
resume_from = None
load_from = f'./pretrained_model/baseline/coco.pth'
workflow = [('train', 1)]
