...
from mmdet_extension.models.utils.build_graph_frcnn import PrototypeComputation
from mmdet_extension.models.utils.discriminator import GradientReversal

from mmdet_extension.models.utils.losses import BCEFocalLoss
from mmdet_extension.models.utils.affinity import Affinity
from mmdet_extension.models.utils.attentions import MultiHeadAttention, HyperGraph
from mmdet_extension.models.utils.sinkhorn_distance import SinkhornDistance
...
class GModule(torch.nn.Module):
    ...
    def __init__(self, opt, in_channels):
        ...
        self.sinkhorn = SinkhornDistance(eps=0.1, max_iter=10, reduction='mean')

        # add by Lvxg:init queue
        self.K = 1000
    ...
    def forward(self, images, features, targets=None, score_maps=None):
        ...
        '''
        if targets:
            features, feat_loss, loss_cont, loss_sink, M_p = self._forward_train(images, features, targets, score_maps)
            return features, feat_loss, loss_cont, loss_sink, M_p
    ...
    def _forward_train(self, images, features, targets=None, score_maps=None):
        ...
        nodes_1, labels_1, weights_1 = self.graph_generator(
            self.compute_locations(features_s), features_s, targets_s
        )
        if targets_t is None:
            nodes_2 = None
        else:
            nodes_2, labels_2, weights_2 = self.graph_generator(
                self.compute_locations(features_t), features_t, targets_t
            )
        ...
        if nodes_2 is not None:  # Both domains have graph nodes
            ...
            # add by Lvxg
            loss_cont, loss_sink = self._forward_organ_loss(nodes_1, nodes_2, labels_1, labels_2)
            ...
            # add by Lvxg
            # nodes_1, nodes_2,labels_1, labels_2, weights_1, weights_2 = \
            #     self.get_one_node(nodes_1, nodes_2,labels_1, labels_2, weights_1, weights_2)
        ...
        if self.matching_cfg != 'none':
            matching_loss_affinity, affinity, M_p = self._forward_aff(nodes_1, nodes_2, labels_1, labels_2)
        ...
        return features, middle_head_loss, loss_cont, loss_sink, M_p
    ...
    def _forward_preprocessing_source_target(self, nodes, labels, weights):
        ...
        for c in labels_exist:
            ...
            if sr_indx.any() and tg_indx.any():  # If the category appear in both domains, we directly collect them!
                ...
                # sr_nodes_c = self.get_non_row(self.source_queue[c])
                # num_now = sr_nodes_c.size(0)
                # if num_now >= num_nodes:
                #     sr_nodes_c = sr_nodes_c[torch.randperm(num_now)[:num_nodes]]
                
                # else:
                #     sr_nodes_c = sr_nodes_c[-1].expand(num_nodes, 256)
                ...
                if self.with_semantic_completion:
                    tg_nodes_c = torch.normal(0, 0.01, size=tg_nodes_c.size()).to(tg_nodes_c.device) + tg_nodes_c if len(
                        sr_nodes_c) < 5 \
                                          ...
                                          std=sr_nodes_c.std(0).unsqueeze(0).expand(sr_nodes_c.size())).to(tg_nodes_c.device)
                else:
                    tg_nodes_c = torch.normal(0, 0.01, size=tg_nodes_c.size()).to(tg_nodes_c.device)

                # tg_nodes_c = self.get_non_row(self.target_queue[c])
                # num_now = tg_nodes_c.size(0)
                # if num_now >= num_nodes:
                #     tg_nodes_c = tg_nodes_c[torch.randperm(num_now)[:num_nodes]]
                
                # else:
                #     tg_nodes_c = tg_nodes_c[-1].expand(num_nodes, 256)
    ...
    def _forward_aff(self, nodes_1, nodes_2, labels_side1, labels_side2):
        if self.matching_cfg == 'o2o':
            ...
            # M_p = threshold_matrix(M)
            M_p = M
            M_p = self.interpolate_tensor(M_p, 300)
            M_p = self.doubly_stochastic_to_permutation(M_p)
    ...
    def doubly_stochastic_to_permutation(self, matrix):
        n = matrix.size(0)
        permutation_matrix = torch.zeros_like(matrix, dtype=torch.int)
        ...
        # 对每一行的排序后的最大值位置设为1
        for i in range(n):
            max_index = sorted_indices[i, -1]
            permutation_matrix[i, max_index] = 1

        return permutation_matrix
    
    def interpolate_tensor(self, input_tensor, target_size):
        # 获取输入张量的大小
        ...
        grid_height = torch.linspace(0, 1, target_height)
        grid_width = torch.linspace(0, 1, target_width)
        grid_y, grid_x = torch.meshgrid(grid_height, grid_width)

        # 将网格转换为与输入大小匹配的范围
        grid_y = grid_y.view(1, target_height, target_width, 1).expand(1, target_height, target_width, 1)
        grid_x = grid_x.view(1, target_height, target_width, 1).expand(1, target_height, target_width, 1)
 
        # 将网格拼接成五维张量
        grid = torch.cat((grid_x, grid_y), dim=3).to(input_tensor.device)

        # 通过插值方法将输入张量插值到目标大小
        interpolated_tensor = F.grid_sample(input_tensor.unsqueeze(0).unsqueeze(0), grid, align_corners=True)

        return interpolated_tensor.squeeze()
    ...
    # add by Lvxg
    def get_one_node(self, node1, node2, label1, label2, weight1=None, weight2=None):
    ...
    def _dequeue_and_enqueue(self, nodes_1, nodes_2, labels_1, labels_2):
        ...
        for idx, cls in enumerate(labels_1.long()):
            ptr = int(self.queue_ptr[cls])
            self.source_queue[cls][ptr] = nodes_1[idx].detach()
            self.target_queue[cls][ptr] = nodes_2[idx].detach()
            ptr = (ptr + 1) % self.K
    ...
    def _forward_organ_loss(self, nodes_s, nodes_t, labels_s, labels_t):
        """
        nodes: source-domain
        input: nodes(list)
        """
        ...
        nodes_1, nodes_2,labels_1, labels_2, weights_1, weights_2 = \
            self.get_one_node(nodes_s, nodes_t,labels_s, labels_t)
        ...
        for idx, cls in enumerate(labels_1.long()):
            if idx == 0:
                continue
            query_s = nodes_1[idx].unsqueeze(0)
            key_s = self.get_non_row(self.source_queue[cls])
            queue_s = self.get_non_row(self.source_queue[0])

            query_t = nodes_2[idx].unsqueeze(0)
            key_t = self.get_non_row(self.target_queue[cls])
            queue_t = self.get_non_row(self.target_queue[0])

            keys = torch.cat([key_s, key_t], dim=0)
            queues = torch.cat([queue_s, queue_t], dim=0)

            cont_loss_1 = self.contrastive_loss(query_s, keys, queues)
            cont_loss_2 = self.contrastive_loss(query_t, keys, queues)
            cont_loss_3 = self.contrastive_loss(query_s, query_t, queues)

            loss_cont += (cont_loss_1 + cont_loss_2 + cont_loss_3)
            loss_sink += self.sinkhorn(query_s, query_t)[0]

        # loss_cont_norm, loss_sink_norm = self.scaler.scale(loss_cont, loss_sink)
        # queue_loss = 0.5 * loss_cont_norm + 0.5 * loss_sink_norm

        # queue_loss = torch.sigmoid(queue_loss)

        return loss_cont, loss_sink
    
    def get_non_row(self, x):
        if torch.all(x==0, dim=1).any().item():
            zero_raw_idx = (x.sum(dim=1) == 0).nonzero(as_tuple=True)[0][0]
            return x[:zero_raw_idx]
        else:
            return x
...
def threshold_matrix(matrix, threshold = 0.5):
    binary_matrix = torch.where(matrix > threshold, torch.tensor(1).to(device=matrix.device), torch.tensor(0).to(device=matrix.device))
